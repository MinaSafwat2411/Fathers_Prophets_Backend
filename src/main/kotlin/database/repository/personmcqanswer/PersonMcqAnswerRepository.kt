package com.fathersprophets.backend.database.repository.personmcqanswer
...
class PersonMcqAnswerRepository(
    ...
    override fun createPersonMcqAnswer(request: CreatePersonMcqAnswerRequest, lang: String): ApiResponse<PersonMcqAnswerResponse> {
        val existing = personMcqAnswerDao.findByQuestionIdAndUserId(request.questionId, request.userId)
        if (existing != null) throw IllegalStateException(Localization.get("person_mcq_answer_already_exists", lang))
        ...
        val created = personMcqAnswerDao.findById(id)
        return ApiResponse(
            success = true,
            data = created?.convertToPersonMcqAnswerResponse(),
    ...
    override fun updatePersonMcqAnswer(id: Int, request: UpdatePersonMcqAnswerRequest, lang: String): ApiResponse<PersonMcqAnswerResponse> {
        personMcqAnswerDao.update(request.convertToPersonMcqAnswerDto(id))
        val updated = personMcqAnswerDao.findById(id)
        return ApiResponse(
            success = true,
            data = updated?.convertToPersonMcqAnswerResponse(),
    ...
    override fun updatePersonMcqAnswerStatus(id: Int, request: UpdateMcqAnswerStatusRequest, lang: String): ApiResponse<PersonMcqAnswerResponse> {
        personMcqAnswerDao.updateStatus(statusToDto(id, AnswerStatus.valueOf(request.status)))
        val updated = personMcqAnswerDao.findById(id)
        return ApiResponse(
            success = true,
            data = updated?.convertToPersonMcqAnswerResponse(),
    ...
    override fun deletePersonMcqAnswer(id: Int, lang: String): ApiResponse<Nothing> {
        personMcqAnswerDao.delete(idToDto(id))
        return ApiResponse(
            success = true,
            data = null,
    ...
    private fun idToDto(id: Int) = PersonMcqAnswerDto(
        id = id,
        answer = "",
        questionId = 0,
        userId = 0,
