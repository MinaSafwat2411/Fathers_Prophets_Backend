...
object OtpTable : Table("otp") {
    ...
    // Holds the new email/phone a user is trying to move to, so the value is bound to the OTP
    // server-side and can't be swapped for a different one at verification time.
    val pendingValue = varchar("pending_value", 255).nullable()
