...
class QuizAnswerDao {
    ...
    fun findByQuestionIdAndUserIdAndDayIdAndQuizId(questionId: Int, userId: Int, dayId: Int, quizId: Int) = transaction {
        QuizAnswersTable.selectAll()
            .where {
                (QuizAnswersTable.questionId eq questionId) and
                    (QuizAnswersTable.userId eq userId) and
                    (QuizAnswersTable.dayId eq dayId) and
                    (QuizAnswersTable.quizId eq quizId)
            }
            .singleOrNull()?.let { resultRowToDto(it) }
    }
    ...
    fun delete(dto: QuizAnswerDto) = transaction {
        QuizAnswersTable.deleteWhere { QuizAnswersTable.id eq dto.id } > 0
    }
}