...
class UserProgressQuizDao {
    ...
    fun findByUserIdAndQuizIdAndDayId(userId: Int, quizId: Int, dayId: Int) = transaction {
    ...
    fun delete(dto: UserProgressQuizDto) = transaction {
        UserProgressQuizTable.deleteWhere { UserProgressQuizTable.id eq dto.id } > 0
    }

    fun incrementScore(userId: Int, quizId: Int, dayId: Int, amount: Int) = transaction {
        val existing = UserProgressQuizTable.selectAll()
            .where {
                (UserProgressQuizTable.userId eq userId) and
                    (UserProgressQuizTable.quizId eq quizId) and
                    (UserProgressQuizTable.dayId eq dayId)
        ...
        if (existing != null) {
            val newScore = existing[UserProgressQuizTable.score] + amount
            UserProgressQuizTable.update({ UserProgressQuizTable.id eq existing[UserProgressQuizTable.id] }) {
                it[score] = newScore
            }
            newScore
        } else {
